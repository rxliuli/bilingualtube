...
const META_COMMENTARY_PATTERN =
  /[（(]注[：:]|原文第?\d+行|原文[本的]?(缺失|不完整|未提供)|此处(待补|保留原编号|为格式化|原文)|(需|请)提供(对应|完整)原文|按(规则|要求)(保留|补齐|输出)|根据(上下文|原文)(推测|补全|补充)|\[此处待补|(或[可意]译为|也可以翻译为|直译为)|(结合语境|结合上下文).{0,10}(更可能|似可|应)|保留(字面意?译|原文或意译)/
