...
function cacheKey(engine: string, targetLang: string, text: string): string {
  return `${engine}-${targetLang}-${text}`
}
...
export async function getCached(
  ...
  const entry = await db.get('translations', cacheKey(engine, targetLang, text))
...
export async function setCached(
  ...
  await db.put('translations', {
    key: cacheKey(engine, targetLang, text),
    text: translated,
    ts: Date.now(),
  })
}
